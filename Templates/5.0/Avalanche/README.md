...
| **Macro Name**                     | **Description**|
| ---------------------------------- | ---------------------- |
|$(ZEN_TRNS_ISOLATION) | Transaction Isolation setting for the connection. Allowed values are 'serializable', 'repeatable read', 'read committed', 'read uncommitted'. |
|$(ZEN_WHERE_STMT) | Where statement to filter source records. Statement should exclude 'WHERE' keyword and be syntactically correct as per Zen's SQL grammer. This macro is not applicable if query statement is used instead of table. |
|$(ZEN_JDBC_DRIVER_JAR_LOCATION) | If you are using a JDBC connection for connecting to Zen, then you can specify zen jdbc driver jar location using this macro. Value can be a path to the directory containing jar files or path to the jar file itself.|
|$(ZEN_CONNECTION_TYPE) | Specify the connection type for connecting to zen. Allowed values are jdbc, odbc.|
